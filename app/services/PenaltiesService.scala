...
class PenaltiesService @Inject()(connector: PenaltiesConnector) {
  ...
  private def findEstimatedVatInterestFromPayload(payload: ETMPPayload): BigDecimal = {
    payload.vatOverview.map {
      estimatedVATInterest => {
        estimatedVATInterest.map(_.estimatedInterest.getOrElse(BigDecimal(0))).foldLeft(BigDecimal(0))(_ + _)
      }
    }
  }.getOrElse(0)
  ...
  private def findCrystalizedInterestFromPayload(payload: ETMPPayload): BigDecimal = {
    payload.vatOverview.map {
      crystalizedInterest => {
        crystalizedInterest.map(_.crystalizedInterest.getOrElse(BigDecimal(0))).foldLeft(BigDecimal(0))(_ + _)
      }
    }
  }.getOrElse(0)
  ...
  def findOverdueVATFromPayload(payload: ETMPPayload): BigDecimal = {
    payload.vatOverview.map {
      allCharges => {
        allCharges.map(_.amount).foldLeft(BigDecimal(0))(_ + _)
      }
    }
  }.getOrElse(0)
  ...
  def findEstimatedLPPsFromPayload(payload: ETMPPayload): (BigDecimal, Boolean) = {
    payload.latePaymentPenalties.map {
      allLPPs => {
        ...
        val estimatedLPPs = allAdditionalPoints.map(_.financial.outstandingAmountDue).foldLeft(BigDecimal(0))(_ + _)
        val crystallisedLPPs = allFinancialPoints.map(_.financial.outstandingAmountDue).foldLeft(BigDecimal(0))(_ + _)
  ...
  def findTotalLSPFromPayload(payload: ETMPPayload): BigDecimal = {
    payload.penaltyPoints.map(_.financial.map(_.outstandingAmountDue)).collect { case Some(x) => x }.foldLeft(BigDecimal(0))(_ + _)
  }
