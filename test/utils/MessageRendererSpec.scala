  ...
  "getMessage" should {
    "load the relevant agent message" when {
      "the user is an agent" in {
        val fakeAgentRequest = User(vrn = "123456789", arn = Some("AGENT1"))(fakeRequest)
      ...
      "apply the arguments when given" in {
        val fakeAgentRequest = User(vrn = "123456789", arn = Some("AGENT1"))(fakeRequest)
    ...
    "load the normal message when the user is a VAT trader" in {
      val fakeVATTraderRequest = User(vrn = "123456789")(fakeRequest)
      val fakeMessageKey = "message.key"
      val result = MessageRenderer.getMessage(fakeMessageKey)(implicitly, fakeVATTraderRequest)
      result shouldBe fakeMessageKey
    }

    "apply the arguments when given for VAT trader" in {
      val fakeVATTraderRequest = User(vrn = "123456789")(fakeRequest)
      val testMessages = Map(
        "default" -> Map(
          "test.message" -> "I am a {0} with no ARN",
          "agent.test.message" -> "I’m an {0} with an ARN")
      )
      val messagesApi = new DefaultMessagesApi(testMessages)
      val messages = messagesApi.preferred(FakeRequest("GET", "/"))
      val fakeMessageKey = "test.message"
