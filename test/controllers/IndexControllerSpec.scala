...
class IndexControllerSpec extends SpecBase {
  ...
  "IndexController" should {
    ...
    "redirectToAppeals" when {
      ...
      "the user wants to appeal an additional penalty" in new Setup(AuthTestModels.successfulAuthResult) {
        val result = Controller.redirectToAppeals("123456789", isAdditional = true)(fakeRequest)

        status(result) shouldBe SEE_OTHER
        redirectLocation(result) shouldBe Some(s"${appConfig.penaltiesAppealsBaseUrl}/initialise-appeal?penaltyId=$penaltyId&isLPP=false&isAdditional=true")
      }
