...
class LateSubmissionPenaltySpec extends SpecBase {
  ...
  val lspAsModel = LateSubmissionPenalty(
    summary = LSPSummary(
      activePenaltyPoints = 10,
      inactivePenaltyPoints = 12,
      regimeThreshold = 10,
  ...
  "LateSubmissionPenalty" should {
    "be readable from JSON" in {
      val result = Json.fromJson(lspAsJson)(LateSubmissionPenalty.format)
      result.isSuccess shouldBe true
      result.get shouldBe lspAsModel
    }

    "be writable to JSON" in {
      val result = Json.toJson(lspAsModel)(LateSubmissionPenalty.format)
      result shouldBe lspAsJson
    }
  }
