  ...
  "summaryCard" when {
    "given a penalty" should {
      ...
      "display 'Payment not yet received' in VAT Payment date" in {
        doc.select("dt").get(1).text() shouldBe "VAT payment date"
        doc.select("dd").get(1).text() shouldBe "Payment not yet received"
      }
      ...
      "display the date in VAT Payment date" in {
        ...
        docVATPaymentDate.select("dt").get(1).text() shouldBe "VAT payment date"
        docVATPaymentDate.select("dd").get(1).text() shouldBe "1 March 2020"
      }
