    ...
    "IndexView" when {
      ...
      "user is on page" must {
        ...
        "populate summary card when user has LPPs with VAT unpaid" in {
          ...
          vatTraderDocWithLPPVATUnpaid.select(Selectors.summaryCardHeaderTag(Selectors.summaryLPPCard)).text shouldBe overdueTag
        ...
        "populate summary card when user has LPPs with additional penalties" in {
          ...
          vatTraderDocLPPAdditionalPenalty.select(Selectors.summaryCardFooterLink(Selectors.summaryLPPCard)).attr("href") shouldBe redirectToAppealUrlForAdditionalPenalty
        }
