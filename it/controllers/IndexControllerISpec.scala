...
class IndexControllerISpec extends IntegrationSpecCommonBase {
  ...
  "GET /appeal-penalty" should {
    ...
    "redirect the user to the appeals service when the penalty is an additional penalty" in {
      val request = buildClientForRequestToApp(uri = "/appeal-penalty?penaltyId=1234&isLPP=false&isObligation=false&isAdditional=true").get()
      await(request).status shouldBe Status.SEE_OTHER
      await(request).header(HeaderNames.LOCATION).get shouldBe "http://localhost:9181/penalties-appeals/initialise-appeal?penaltyId=1234&isLPP=false&isAdditional=true"
    }
